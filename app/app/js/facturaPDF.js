...
export function generarFactura(orden){
  const doc = new jsPDF();
  doc.setFontSize(18); doc.text("FACTURA TALLERPRO360",20,20);
  doc.setFontSize(12);
  ...
  orden.acciones.forEach(a=>{
    doc.text(`${a.descripcion} - $${a.costo.toLocaleString("es-CO")}`,20,y); y+=10;
  });

  doc.text(`TOTAL: $${orden.total.toLocaleString("es-CO")}`,20,y+20);
