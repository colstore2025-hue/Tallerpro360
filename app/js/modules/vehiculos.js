/**
 * vehiculos.js - TallerPRO360 NEXUS-X V17.1 🏎️
 * NÚCLEO DE INTELIGENCIA DE ACTIVOS Y HOJA DE VIDA TÉCNICA
 * Reforma: Unificación de rutas de datos y Sincronía Total
 */
import { 
    collection, query, where, orderBy, onSnapshot, serverTimestamp, addDoc, doc, setDoc 
} from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
import { db } from "../core/firebase-config.js";
import { hablar } from "../voice/voiceCore.js";

export default async function vehiculosModule(container, state) {
    // 🛡️ Mantenemos coherencia con los otros scripts
    ...
    function escucharVehiculos() {
        ...
        // 🛠️ CORRECCIÓN DE RUTA: Apuntamos a la colección raíz 'vehiculos' filtrando por empresaId
        // Esto asegura que lo que guardas en ordenes.js se vea aquí.
        const q = query(
        ...
        grid.innerHTML = data.map(v => {
            ...
            <div class="group bg-[#0d1117] p-8 rounded-[3.5rem] border border-white/5 hover:border-indigo-500/50 transition-all duration-500 relative overflow-hidden shadow-2xl">
                ...
                <div class="space-y-6 mb-10 relative">
                    <div>
                        <p class="text-[8px] text-slate-600 font-black orbitron uppercase tracking-[0.3em] mb-2 italic">Especificaciones</p>
                        <p class="text-sm font-black text-white uppercase italic tracking-tight">
                            ${v.marca || 'GENÉRICO'} <span class="text-indigo-400">${v.linea || ''}</span>
                        </p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-black/40 p-4 rounded-2xl border border-white/5">
                            <p class="text-[7px] text-slate-600 font-black orbitron uppercase">Odómetro</p>
                    ...
                    <div>
                        <p class="text-[7px] text-slate-600 font-black orbitron uppercase mb-1 italic">Propietario:</p>
                        <p class="text-[10px] font-bold text-slate-300 uppercase truncate">${v.clienteNombre || 'CLIENTE NEXUS'}</p>
                    </div>
                </div>
    ...
    async function abrirModalVehiculo() {
            ...
            html: `
                <div class="space-y-6 p-6 text-left">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[8px] text-indigo-400 font-black orbitron uppercase ml-4 italic">Identidad (Placa)</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[8px] text-slate-500 font-black orbitron uppercase ml-4 italic">Año</label>
                    ...
                    <div class="bg-indigo-500/5 p-6 rounded-[2rem] border border-indigo-500/10">
                        <label class="text-[8px] text-indigo-400 font-black orbitron uppercase ml-2 italic">Odómetro Actual (KM)</label>
                    ...
                    <input id="v-cli" class="w-full bg-black/60 p-6 rounded-3xl text-white border border-white/5 outline-none font-bold uppercase" placeholder="DNI O NOMBRE CLIENTE">
                </div>`,
        ...
        if(form) { 
            try {
                // 🛠️ USAMOS setDoc con el ID de la PLACA para evitar duplicados y facilitar la búsqueda
                await setDoc(doc(db, "vehiculos", form.placa), form);
                hablar(`Activo ${form.placa} registrado en la flota.`);
                window.Swal.fire({ icon: 'success', title: 'RADAR SINCRONIZADO', background: '#010409', color: '#fff' });
            } catch (e) {
                console.error("Error:", e);
                window.Swal.fire({ title: 'Fallo de Enlace', icon: 'error' });
            }
        }
    }
