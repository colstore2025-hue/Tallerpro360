...
if(summaryEl) summaryEl.innerHTML = &lt;div class="grid grid-cols-2 gap-4 border-t border-cyan-500/30 pt-6 mt-6"&gt; &lt;div class="text-[10px] orbitron text-slate-500 uppercase font-black"&gt;BASE: &lt;span class="text-white"&gt;${Math.round(base).toLocaleString()}</span></div>
<div class="text-[10px] orbitron text-slate-500 text-right uppercase font-black">IVA (19%): <span class="text-white">${Math.round(iva).toLocaleString()}&lt;/span&gt;&lt;/div&gt; &lt;div class="text-green-400 font-black text-2xl orbitron italic leading-none"&gt;EBITDA: ${Math.round(ebitda).toLocaleString()}</div>
<div class="text-red-500 font-black text-2xl orbitron text-right italic leading-none font-black">SALDO: ${Math.round(saldo).toLocaleString()}&lt;/div&gt; &lt;/div>;
};
...
msj += linkNexus + "%0A%0A";
msj += "Powered by TallerPRO360 Core";
...
const renderBase = () => {
container.innerHTML = &lt;div class="p-6 lg:p-12 bg-[#05070a] min-h-screen text-slate-100 font-sans pb-40"&gt; &lt;header class="flex flex-col lg:flex-row justify-between items-end gap-6 mb-16 border-b-2 border-red-600 pb-10"&gt; &lt;div class="space-y-1"&gt; &lt;h1 class="orbitron text-7xl font-black italic tracking-tighter text-white uppercase"&gt;NEXUS&lt;span class="text-red-600"&gt;_X&lt;/span&gt;&lt;/h1&gt; &lt;p class="text-[10px] orbitron text-cyan-400 font-bold tracking-[0.8em] uppercase italic"&gt;Automotive Titan Logistics&lt;/p&gt; &lt;/div&gt; &lt;button id="btnNewMission" class="px-12 py-5 bg-cyan-500 text-black rounded-none orbitron text-[10px] font-black hover:bg-white transition-all shadow-[10px_10px_0px_rgba(0,242,255,0.3)]"&gt;INICIAR MISIÓN +&lt;/button&gt; &lt;/header&gt; &lt;div id="grid-ordenes" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8"&gt;&lt;/div&gt; &lt;div id="nexus-terminal" class="hidden fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl p-6 lg:p-12 overflow-y-auto custom-scroll"&gt;&lt;/div&gt; &lt;/div>;
...
modal.innerHTML = &lt;div class="max-w-[1600px] mx-auto pb-40 animate-in zoom-in duration-300"&gt; &lt;div class="flex flex-col lg:flex-row justify-between items-center mb-12 bg-[#0d1117] p-10 border-l-8 border-cyan-500 rounded-r-3xl gap-6"&gt; &lt;div class="flex items-center gap-6"&gt; &lt;input id="f-placa" value="${ordenActiva.placa}" class="bg-black text-7xl font-black orbitron text-white w-80 uppercase border-2 border-white/5 rounded-xl text-center" placeholder="PLACA">
...
latex
{Object.entries(NEXUS_ASCENSOR).map(([k,v]) =&gt; `<option value="

{k}" latex
{ordenActiva.tipo_orden === k ? 'selected' : ''} class="bg-[#0d1117]"&gt;

{v.label.toUpperCase()}</option>).join('')} &lt;/select&gt; &lt;/div&gt; &lt;div class="flex items-center gap-4 bg-black/50 p-4 px-6 rounded-2xl border border-white/5"&gt; &lt;span class="orbitron text-[9px] text-slate-500 font-black uppercase"&gt;Fase Misión:&lt;/span&gt; &lt;select id="f-estado" onchange="window.cambiarEstado(this.value)" class="bg-transparent text-cyan-400 orbitron font-black text-xl outline-none"&gt; ${['COTIZACION', 'INGRESO', 'DIAGNOSTICO', 'REPARACION', 'LISTO', 'ENTREGADO'].map(e => <option value="${e}" latex
{ordenActiva.estado === e ? 'selected' : ''} class="bg-[#0d1117]"&gt;
...
<input id="f-cliente" value=" {ordenActiva.telefono || ''}" placeholder="WHATSAPP (+57)" class="w-full bg-black p-4 mb-6 text-green-400 font-bold border border-white/10 rounded-xl">
...
<h2 id="total-factura" class="orbitron text-[8rem] font-black text-white italic leading-none">$ 0&lt;/h2&gt; &lt;div id="finance-summary" class="w-80"&gt;&lt;/div&gt; &lt;/div&gt; &lt;div id="items-container" class="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scroll"&gt;&lt;/div&gt; &lt;div class="grid grid-cols-3 gap-4 mt-12"&gt; &lt;button onclick="window.agregarDesdeInventario()" class="py-6 border-2 border-cyan-500/50 orbitron text-[10px] font-black text-cyan-400 hover:bg-cyan-500 hover:text-black transition-all rounded-2xl"&gt;&lt;i class="fas fa-search text-xl"&gt;&lt;/i&gt; LUPA STOCK&lt;/button&gt; &lt;button onclick="window.addItemNexus('REPUESTO')" class="py-6 border-2 border-white/10 orbitron text-[10px] font-black text-white hover:bg-white hover:text-black transition-all rounded-2xl"&gt;&lt;i class="fas fa-box-open text-xl"&gt;&lt;/i&gt; EXTERNO&lt;/button&gt; &lt;button onclick="window.addItemNexus('MANO_OBRA')" class="py-6 border-2 border-red-600/20 orbitron text-[10px] font-black text-red-600 hover:bg-red-600 hover:text-white transition-all rounded-2xl"&gt;&lt;i class="fas fa-tools text-xl"&gt;&lt;/i&gt; LABOR&lt;/button&gt; &lt;/div&gt; &lt;/div&gt; &lt;div class="grid grid-cols-3 gap-6"&gt; &lt;div class="bg-black p-6 rounded-3xl border border-white/5"&gt;&lt;label class="orbitron text-[9px] text-slate-500 block mb-2 uppercase"&gt;Insumos (IVA)&lt;/label&gt;&lt;input id="f-insumos-iva" value="${ordenActiva.insumos || 0}" type="number" onchange="window.recalcularFinanzas()" class="bg-transparent text-white text-4xl font-black w-full outline-none"></div>
<div class="bg-black p-6 rounded-3xl border border-white/5"><label class="orbitron text-[9px] text-yellow-500 block mb-2 uppercase">Insumos (NO IVA)</label><input id="f-insumos-no-iva" value="${ordenActiva.insumos_no_iva || 0}" type="number" onchange="window.recalcularFinanzas()" class="bg-transparent text-yellow-500 text-4xl font-black w-full outline-none"&gt;&lt;/div&gt; &lt;div class="bg-black p-6 rounded-3xl border border-white/5"&gt;&lt;label class="orbitron text-[9px] text-green-500 block mb-2 uppercase font-black"&gt;Anticipo&lt;/label&gt;&lt;input id="f-anticipo" value="${ordenActiva.anticipo || 0}" type="number" onchange="window.recalcularFinanzas()" class="bg-transparent text-green-400 text-4xl font-black w-full outline-none"></div>
</div>
...
const ops = {};
snap.forEach(d => { ops[d.id] = ``${d.data().nombre.toUpperCase()} | STOCK:  {Number(d.data().precioVenta).toLocaleString()}`; });
...
conceptoContable = CIERRE TOTAL SERVICIO: ${placa}; } else if (vAnticipo &gt; 0) { montoContable = vAnticipo; conceptoContable =ANTICIPO RECIBIDO: ${placa};
}
...
return &lt;div onclick="window.abrirTerminalNexus('${d.id}')"
...
<div class="mb-6">
<h4 class="orbitron text-4xl font-black text-white group-hover:text-cyan-400 transition-colors tracking-tighter"> {info.cliente || 'OPERACIÓN_ANÓNIMA'}</p>
</div>
...
<span class="text-[9px] orbitron font-black  {info.estado}
</span>
</div>
...
<span class="text-[11px] orbitron font-black  {saldoReal.toLocaleString()}
</span>
</div>
</div>
...
style="background:  {config.color}">
 {info.tipo_orden === 'DIAGNOSTICO' ? 'SCANNING' : 'REPAIRING'}</span>
...
c.innerHTML = ordenActiva.items.map((it, i) => &lt;div class="flex flex-col gap-3 bg-white/[0.03] p-5 rounded-2xl border border-white/10 mb-4 relative overflow-hidden group"&gt; &lt;div class="absolute top-0 left-0 h-full w-1 ${it.tipo === 'REPUESTO' ? 'bg-cyan-500' : 'bg-red-600'}"></div>
...
<button onclick="window.removeItemNexus(${i})" class="text-slate-600 hover:text-red-500"&gt;&lt;i class="fas fa-trash-alt"&gt;&lt;/i&gt;&lt;/button&gt; &lt;/div&gt; &lt;div class="grid grid-cols-4 gap-4 items-end"&gt; &lt;div&gt;&lt;label class="text-[7px] orbitron text-slate-500 block uppercase"&gt;Costo&lt;/label&gt;&lt;input type="number" onchange="window.updateItem(${i}, 'costo', this.value)" value="${it.costo}" class="bg-black/50 p-2 text-red-400 text-xs rounded w-full border border-white/5 outline-none font-black orbitron"&gt;&lt;/div&gt; &lt;div&gt;&lt;label class="text-[7px] orbitron text-green-500 block uppercase"&gt;Venta&lt;/label&gt;&lt;input type="number" onchange="window.updateItem(${i}, 'venta', this.value)" value="${it.venta}" class="bg-black/50 p-2 text-green-400 text-xs rounded w-full border border-white/5 outline-none font-black orbitron"&gt;&lt;/div&gt; &lt;div&gt;&lt;label class="text-[7px] orbitron text-slate-500 block uppercase font-black"&gt;Cant&lt;/label&gt;&lt;input type="number" onchange="window.updateItem(${i}, 'cantidad', this.value)" value="${it.cantidad || 1}" class="bg-black/50 p-2 text-white text-xs rounded w-full border border-white/5 outline-none font-black orbitron"&gt;&lt;/div&gt; &lt;div class="text-right text-xs font-black orbitron text-white italic"&gt;$${(it.venta * (it.cantidad || 1)).toLocaleString()}&lt;/div&gt; &lt;/div&gt; &lt;/div>).join('');
};
