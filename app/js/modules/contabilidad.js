...
export default async function contabilidad(container) {
    ...
    // --- MOTOR DE CLASIFICACIÓN CUÁNTICA (Normalización Estricta) ---
    const clasificarMovimiento = (m) => {
        const t = (m.tipo || "").toLowerCase();
        if (t.includes("ingreso") || t.includes("4135") || t.includes("saneamiento") || t.includes("1105") || t.includes("capital") || t.includes("2805")) {
            return "INGRESO";
        }
        if (t.includes("gasto") || t.includes("51") || t.includes("nomina") || t.includes("arriendo") || t.includes("servicio") || t.includes("compra")) {
            return "GASTO";
        }
        return "OTRO";
    };
    ...
    function escucharDatos() {
        ...
        unsubscribe = onSnapshot(q, (snap) => {
            ...
            list.innerHTML = docs.map(m => {
                ...
                // Cálculo de Cartera Pendiente (1305 vs 1105)
