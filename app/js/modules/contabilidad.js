/**
 * contabilidad.js - NEXUS-X MASTER-CORE V21.5-PRO 🏛️
 * Motor Financiero TallerPRO360 - Blindaje Total
...
// Decidí integrar el saneamiento directamente aquí para evitar errores de importación/bloqueo
const NexusSaneamiento = (data) => {
    return {
        ...data,
        monto: parseFloat(data.monto || 0), // Asegura que siempre sea número para cálculos
...
export default async function contabilidad(container) {
    ...
    const MAP_TIPOS = {
        ...
        gastos: ['gasto_operativo', 'gasto_insumos', 'nomina_pago', 'nomina', 'gasto'], // 'gasto' añadido para fallback
            ...
            <header class="flex flex-col lg:flex-row justify-between items-center gap-8 mb-12 border-b border-white/10 pb-10">
                <div class="text-center lg:text-left">
                    <h1 class="orbitron text-5xl font-black text-white italic tracking-tighter uppercase">
                        FINANCE <span class="text-amber-400">CORE</span><span class="text-cyan-500 text-xl">.V21.5</span>
                    </h1>
                    <p class="text-[9px] text-slate-500 font-black uppercase tracking-[0.4em] orbitron mt-2 italic">TallerPRO360 Global Control</p>
                </div>
            ...
            <div class="lg:col-span-4">
                <div class="bg-[#0d1117] p-8 rounded-[3rem] border border-white/5 shadow-2xl sticky top-10">
                    ...
                    <div class="space-y-5">
                        ...
                        <button id="btnGuardarFinanza" class="w-full bg-amber-400 text-black font-black orbitron py-5 rounded-2xl hover:scale-105 transition-all uppercase tracking-widest">Sincronizar Libro +</button>
                    </div>
                </div>
            </div>
    ...
    function escucharContabilidad() {
        ...
        unsubscribe = onSnapshot(q, (snap) => {
            ...
            list.innerHTML = snap.docs.map(docSnap => {
                ...
                const fecha = m.creadoEn ? m.creadoEn.toDate().toLocaleString() : 'Sincronizando...';

                return `
                <div class="bg-[#0d1117] p-6 rounded-[2.5rem] border border-white/5 flex justify-between items-center group hover:bg-white/[0.05] transition-all">
    ...
    const cargarVistaCuentas = async () => {
        ...
        <div class="bg-[#0d1117] p-10 rounded-[4rem] border border-white/5 shadow-3xl">
            <h3 class="orbitron text-xl font-black mb-10 text-amber-400 italic text-center">BALANCE OFICIAL PUC</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="p-8 bg-black/40 rounded-[3rem] border border-emerald-500/10">
                    <p class="text-[9px] orbitron font-black text-emerald-400 mb-6 uppercase tracking-widest">Ingresos (4135)</p>
                    <div class="space-y-3" id="puc-clase-4"></div>
                </div>
                <div class="p-8 bg-black/40 rounded-[3rem] border border-red-500/10">
                    <p class="text-[9px] orbitron font-black text-red-500 mb-6 uppercase tracking-widest">Gastos (5105/5195)</p>
                    <div class="space-y-3" id="puc-clase-5"></div>
                </div>
                <div class="p-8 bg-black/40 rounded-[3rem] border border-amber-500/10">
                    <p class="text-[9px] orbitron font-black text-amber-500 mb-6 uppercase tracking-widest">Activos (1105)</p>
    ...
    const calcularPUC = async () => {
        ...
        const row = (c, v) => `<div class="flex justify-between border-b border-white/5 pb-2"><span class="text-[9px] text-slate-400 font-mono">${c}</span><span class="text-[10px] text-white font-black">$ ${v.toLocaleString()}</span></div>`;
    ...
    const obtenerEstilo = (tipo) => {
        ...
        return estilos[tipo] || { icon: 'fas fa-receipt', text: 'text-slate-400', bg: 'bg-slate-500/10', border: 'border-slate-500/20' };
    };
    ...
    async function registrarMovimiento() {
        ...
        // MANIOBRA NEXUS INTEGRADA: Saneamiento sin archivos externos que bloqueen
        const datosSaneados = NexusSaneamiento({
            empresaId,
            concepto: cIn.value,
            tipo: tIn.value,
            monto: monto,
