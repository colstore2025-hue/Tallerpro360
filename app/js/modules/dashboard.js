/**
 * dashboard.js
 * PRO360 Dashboard · MODO DIOS GUARDIAN
 * Integración completa con Firestore Guardian IA y Orquestador
 */

import { collection, getDocs, query, onSnapshot } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
export default async function dashboard(container, state) {
  ...
  // Activar Guardian IA para corrección y logs
  activarModoDiosGuardian(state.empresaId);

  // Watcher Firestore: actualización en tiempo real
  const ref = collection(db, `${base}/ordenes`);
  onSnapshot(ref, () => cargarDashboard());
  ...
  async function cargarDashboard() {
    try {
      ...
      snapshot.forEach(docSnap => {
        ...
        const total = Number(d.valorTrabajo || d.total || 0);
        ...
        // Fecha segura
        let fecha = "sin_fecha";
        try { if (d.creadoEn?.toDate) fecha = d.creadoEn.toDate().toISOString().split("T")[0]; } catch {}

        ingresosPorDia[fecha] = (ingresosPorDia[fecha] || 0) + total;

        // Alertas seguras
        try { const a = JSON.parse(d.alertasIA || "[]"); if (Array.isArray(a)) alertas.push(...a); } catch {}
        // Recomendaciones seguras
        try { const r = JSON.parse(d.diagnosticoIA?.recomendaciones || "[]"); if (Array.isArray(r)) recomendaciones.push(...r); } catch {}
      });
      ...
      // Panel IA
      renderIA({
        ingresos, costos, utilidad, ordenes, alertas, recomendaciones
      });
  ...
  function renderGrafica(data) {
    ...
    new Chart(ctx, {
      ...
      options: { responsive:true, plugins:{ legend:{ labels:{ color:"#00ffcc" } } } }
    });
  }
  ...
  function renderIA(data) {
    ...
    const margen = data.ingresos ? ((data.utilidad / data.ingresos)*100).toFixed(2) : 0;
    const ticket = data.ordenes ? (data.ingresos / data.ordenes).toFixed(0) : 0;
    ...
    panel.innerHTML = `
      <div style="background:#0f172a;padding:30px;border-radius:16px;">
        ...
        <button id="vozGerente" style="margin-top:20px;padding:12px;background:#00ff99;border:none;border-radius:10px;font-weight:bold;">
          🎤 Escuchar IA
        </button>
      </div>
    `;
    ...
    // Render sugerencias IA Orquestador
    try {
      renderSugerencias("iaPanel", generarSugerencias({ dashboardData: data, empresaId: state.empresaId }));
    } catch(e){ console.warn("IA Advisor error:", e); }
  }
  ...
  function ordenarDatos(obj) {
    return Object.fromEntries(
      Object.entries(obj).filter(([k])=>k!=="sin_fecha").sort(([a],[b])=>new Date(a)-new Date(b))
    );
  }
