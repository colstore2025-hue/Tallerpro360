 ...
 * ARQUITECTURA TIPO ERP PRO-2030 TERMINATOR
...
export default async function inventario(container) {
    ...
    const switchTab = (tipo) => {
        ...
        btnP.className = `flex-1 lg:flex-none px-10 py-4 rounded-[2rem] text-[10px] font-black uppercase transition-all orbitron ${tipo === 'PROPIO' ? active : inactive}`;
        btnC.className = `flex-1 lg:flex-none px-10 py-4 rounded-[2rem] text-[10px] font-black uppercase transition-all orbitron ${tipo === 'CLIENTE' ? active : inactive}`;
...
async function abrirModalCarga() {
    ...
    window.buscarEnInventario = async (idx) => {
        const q = query(collection(db, "inventario"), where("empresaId", "==", empresaId), where("origen", "==", "PROPIO"), orderBy("nombre", "asc"));
        const snap = await getDoc(q); // Aquí se puede optimizar con un getDocs para el listado

        const { value: selectedId } = await window.Swal.fire({
            title: 'SELECCIONAR REPUESTO',
            background: '#010409', color: '#fff',
            input: 'select',
            inputOptions: Object.fromEntries(snap.docs.map(d => [d.id, `${d.data().nombre} [Stock: ${d.data().cantidad}]`])),
        ...
        if (selectedId) {
            ...
            if (item.cantidad <= 0) {
                Swal.fire('BLOQUEADO', 'No hay stock disponible', 'error');
                return;
            }
            // Emitir evento para ordenes.js
            window.dispatchEvent(new CustomEvent('piezaSeleccionada', { detail: { idx, item: { ...item, id: selectedId } } }));
        }
    };
    ...
    window.ajustarStock = async (id, cambio) => {
        ...
        const nuevaCantidad = Math.max(0, (data.cantidad || 0) + cambio);
