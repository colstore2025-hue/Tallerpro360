...
exports.crearPlanCuentas = async (empresaId) => {
  if (!empresaId) throw new functions.https.HttpsError("invalid-argument","empresaId es obligatorio");

  const empresaRef = db.collection("empresas").doc(empresaId);
  const empresaDoc = await empresaRef.get();
  if (!empresaDoc.exists) throw new functions.https.HttpsError("not-found","La empresa no existe");
  ...
  const cuentasRef = empresaRef.collection("contabilidad").doc("cuentas").collection("lista");
  const snapshot = await cuentasRef.limit(1).get();
  if (!snapshot.empty) return true; // Ya existe plan de cuentas
...
========================================== */
exports.registrarAsiento = async ({ empresaId, tipo, referenciaId, movimientos, creadoPor }) => {

  if (!empresaId || !movimientos || !Array.isArray(movimientos)) {
    throw new functions.https.HttpsError("invalid-argument","Datos incompletos para registrar asiento");
  }
  if (movimientos.length < 2) {
    throw new functions.https.HttpsError("failed-precondition","Un asiento debe tener mínimo dos movimientos");
  }
  ...
  movimientos.forEach(m => {
    if (!m.cuenta) throw new functions.https.HttpsError("invalid-argument","Cada movimiento debe tener cuenta");

    const debe = Number(m.debe || 0);
    const haber = Number(m.haber || 0);

    if (debe < 0 || haber < 0) throw new functions.https.HttpsError("invalid-argument","Los valores no pueden ser negativos");
  ...
  if (totalDebe !== totalHaber) throw new functions.https.HttpsError("failed-precondition","El asiento no está balanceado");
